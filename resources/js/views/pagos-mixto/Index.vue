        ...
        <div class="card">
            <div class="card-header bg-primary d-flex align-items-center">
                <h5 class="card-title mb-0 text-white flex-grow-1">
                    <i class="fas fa-exchange-alt mr-1"></i> Pagos Mixtos
                </h5>
                <button type="button" class="btn btn-light btn-sm" @click="openCreateModal">
                    <i class="fas fa-plus-circle mr-1"></i> Nuevo Pago Mixto
                </button>
            </div>
            <div class="card-body">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" v-model="search" placeholder="Buscar pago mixto..." @keyup.enter="loadItems(1)">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-primary">
                        <h5 class="modal-title text-white"><i class="fas fa-plus"></i> Nuevo Pago Mixto</h5>
                    ...
                    <form @submit.prevent="createItem">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>Monto</label>
                                <input type="number" class="form-control" :class="{ 'is-invalid': formErrors.monto }" v-model="createForm.monto" step="0.01" min="0">
                                <div class="invalid-feedback" v-if="formErrors.monto">{{ formErrors.monto[0] }}</div>
                            </div>
                        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-warning">
                        <h5 class="modal-title text-white"><i class="fas fa-edit"></i> Editar Pago Mixto</h5>
                    ...
                    <form @submit.prevent="updateItem">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>Monto</label>
                                <input type="number" class="form-control" :class="{ 'is-invalid': formErrors.monto }" v-model="editForm.monto" step="0.01" min="0">
                                <div class="invalid-feedback" v-if="formErrors.monto">{{ formErrors.monto[0] }}</div>
                            </div>
                        </div>
                        <div class="modal-footer">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-info">
                        <h5 class="modal-title text-white"><i class="fas fa-eye"></i> Detalle Pago Mixto</h5>
...
import pagosMixtoService from '@/services/pagosMixto';
...
const createForm = reactive({ recibo: '', fecha: '', tipo_pago: '', monto: '' });
const editForm = reactive({ recibo: '', fecha: '', tipo_pago: '', monto: '' });
...
async function loadItems(page = 1) {
    loading.value = true;
    try {
        const res = await pagosMixtoService.index({ page, search: search.value });
...
async function createItem() {
    ...
    Object.keys(formErrors).forEach(k => delete formErrors[k]);
    try {
        await pagosMixtoService.store(createForm);
...
async function updateItem() {
    ...
    Object.keys(formErrors).forEach(k => delete formErrors[k]);
    try {
        await pagosMixtoService.update(currentItem.value.id, editForm);
...
async function deleteItem(item) {
    ...
    try {
        await pagosMixtoService.destroy(item.id);
