        ...
        <div class="card">
            <div class="card-header bg-primary d-flex align-items-center">
                <h5 class="card-title mb-0 text-white flex-grow-1">
                    <i class="fas fa-user-cog mr-1"></i> Lista de Usuarios
                </h5>
                <button v-if="hasPermission('crear-usuario')" type="button" class="btn btn-light btn-sm" @click="openCreateModal">
                            ...
                            <tr v-for="(item, index) in items.data" :key="item.id" class="text-center">
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        ...
                                        <button v-if="hasPermission('editar-usuario')" class="btn btn-warning btn-sm" title="Editar" @click="openEditModal(item)">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button v-if="hasPermission('borrar-usuario')" class="btn btn-danger btn-sm" title="Eliminar" @click="deleteItem(item)">
        ...
        <div class="modal fade" :class="{ show: showCreate, dBlock: showCreate }" tabindex="-1" style="background: rgba(0,0,0,0.5);" v-if="showCreate">
            <div class="modal-dialog"><div class="modal-content">
                <div class="modal-header bg-primary">
                    <h5 class="modal-title text-white"><i class="fas fa-plus"></i> Nuevo Usuario</h5>
                ...
                <form @submit.prevent="createItem">
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" v-model="createForm.name" class="form-control" :class="{ 'is-invalid': formErrors.name }" required>
                            <div class="invalid-feedback" v-if="formErrors.name">{{ formErrors.name[0] }}</div>
                        </div>
                        <div class="form-group">
                            <label>Email <span class="text-danger">*</span></label>
                            <input type="email" v-model="createForm.email" class="form-control" :class="{ 'is-invalid': formErrors.email }" required>
                            <div class="invalid-feedback" v-if="formErrors.email">{{ formErrors.email[0] }}</div>
                        </div>
                        <div class="form-group">
                            <label>Contraseña <span class="text-danger">*</span></label>
                            <input type="password" v-model="createForm.password" class="form-control" :class="{ 'is-invalid': formErrors.password }" required>
                            <div class="invalid-feedback" v-if="formErrors.password">{{ formErrors.password[0] }}</div>
                        </div>
                        <div class="form-group">
                            <label>Confirmar Contraseña</label>
                            <input type="password" v-model="createForm.password_confirmation" class="form-control" :class="{ 'is-invalid': formErrors.password_confirmation }">
                            <div class="invalid-feedback" v-if="formErrors.password_confirmation">{{ formErrors.password_confirmation[0] }}</div>
                        </div>
                        <div class="form-group">
                            <label>Rol</label>
                            <select v-model="createForm.role" class="form-control" :class="{ 'is-invalid': formErrors.role }">
            ...
            <div class="modal-dialog"><div class="modal-content">
                ...
                <form @submit.prevent="updateItem">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Email <span class="text-danger">*</span></label>
                            <input type="email" v-model="editForm.email" class="form-control" :class="{ 'is-invalid': formErrors.email }" required>
                            <div class="invalid-feedback" v-if="formErrors.email">{{ formErrors.email[0] }}</div>
                        </div>
                        <div class="form-group">
                            <label>Contraseña <small class="text-muted">(dejar vacío para no cambiar)</small></label>
                            <input type="password" v-model="editForm.password" class="form-control" :class="{ 'is-invalid': formErrors.password }">
                            <div class="invalid-feedback" v-if="formErrors.password">{{ formErrors.password[0] }}</div>
                        </div>
                        <div class="form-group">
                            <label>Confirmar Contraseña</label>
                            <input type="password" v-model="editForm.password_confirmation" class="form-control" :class="{ 'is-invalid': formErrors.password_confirmation }">
                            <div class="invalid-feedback" v-if="formErrors.password_confirmation">{{ formErrors.password_confirmation[0] }}</div>
                        </div>
                        <div class="form-group">
                            <label>Rol</label>
                            <select v-model="editForm.role" class="form-control" :class="{ 'is-invalid': formErrors.role }">
                                <option value="">Seleccionar rol</option>
                                <option value="admin">Admin</option>
                                <option value="user">User</option>
                                <option value="vendedor">Vendedor</option>
        ...
        <div class="modal fade" :class="{ show: showView, dBlock: showView }" tabindex="-1" style="background: rgba(0,0,0,0.5);" v-if="showView">
            <div class="modal-dialog"><div class="modal-content">
                <div class="modal-header bg-info">
                    <h5 class="modal-title text-white"><i class="fas fa-eye"></i> Detalle Usuario</h5>
                ...
                <div class="modal-body" v-if="currentItem">
                    <div class="row mb-2">
                        ...
                        <div class="col-8">{{ currentItem.id }}</div>
                    </div>
                    <div class="row mb-2">
                        <div class="col-4 font-weight-bold">Nombre:</div>
                        <div class="col-8">{{ currentItem.name }}</div>
                    </div>
                    <div class="row mb-2">
                        <div class="col-4 font-weight-bold">Email:</div>
                        <div class="col-8">{{ currentItem.email }}</div>
                    </div>
                    <div class="row mb-2">
                        <div class="col-4 font-weight-bold">Email verificado:</div>
                        <div class="col-8">{{ currentItem.email_verified_at ? 'Sí' : 'No' }}</div>
                    </div>
                    <div class="row mb-2">
                        <div class="col-4 font-weight-bold">Roles:</div>
                        <div class="col-8">{{ currentItem.roles?.map(r => r.name).join(', ') || '-' }}</div>
                    </div>
                    <div class="row mb-2" v-if="currentItem.created_at">
                        <div class="col-4 font-weight-bold">Creado:</div>
                        <div class="col-8">{{ formatDate(currentItem.created_at) }}</div>
                    </div>
                </div>
                <div class="modal-footer">
...
const createForm = reactive({ name: '', email: '', password: '', password_confirmation: '', role: '' });
const editForm = reactive({ name: '', email: '', password: '', password_confirmation: '', role: '' });
...
async function createItem() {
    ...
    Object.keys(formErrors).forEach(k => delete formErrors[k]);
    try {
        await userService.store(createForm);
...
function openEditModal(item) {
    ...
    editForm.role = item.roles?.[0]?.name || '';
    Object.keys(formErrors).forEach(k => delete formErrors[k]);
    showEdit.value = true;
}
...
async function updateItem() {
    ...
    try {
        ...
        await userService.update(currentItem.value.id, data);
...
async function deleteItem(item) {
    ...
    try { await userService.destroy(item.id); success.value = 'Registro eliminado exitosamente'; loadItems(currentPage.value); }
