    ...
    <div>
        ...
        <div class="card">
            <div class="card-header bg-primary d-flex align-items-center">
                ...
                <button v-if="hasPermission('crear-rol')" type="button" class="btn btn-light btn-sm" @click="openCreateModal">
                    <i class="fas fa-plus-circle mr-1"></i> Nuevo Rol
                </button>
            </div>
            <div class="card-body">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" v-model="search" placeholder="Buscar rol..." @keyup.enter="loadItems(1)">
                            ...
                            <tr v-for="(item, index) in items.data" :key="item.id" class="text-center">
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        ...
                                        <button v-if="hasPermission('editar-rol')" class="btn btn-warning btn-sm" title="Editar" @click="openEditModal(item)">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button v-if="hasPermission('borrar-rol')" class="btn btn-danger btn-sm" title="Eliminar" @click="deleteItem(item)">
        ...
        <div class="modal fade" :class="{ show: showEdit, dBlock: showEdit }" tabindex="-1" style="background: rgba(0,0,0,0.5);" v-if="showEdit">
            <div class="modal-dialog"><div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title text-white"><i class="fas fa-edit"></i> Editar Rol</h5>
...
async function loadItems(page = 1) {
    loading.value = true;
    try {
        const res = await roleService.index({ page, search: search.value });
